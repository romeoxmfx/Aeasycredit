...
public class PhotoSelectorActivity extends Activity implements
    ...
    public void selectAll() {
        if(photos != null && photos.size() > 0){
            ...
            if (selected.size() == photos.size()) {
                btSelectAll.setText(getResources().getString(R.string.unselect_all));
            } else {
                btSelectAll.setText(getResources().getString(R.string.select_all));
            }
            ...
            if (photos != null && photos.size() > 0) {
                ...
                for (PhotoModel model : photos) {
                    ...
                    if (!TextUtils.isEmpty(selectedString)
                            && selectedString.contains(model.getOriginalPath())) {
                        model.setChecked(true);
                        if (!selected.contains(model))
                            selected.add(model);
                        i++;
