...
public class SplashActivity extends BaseActivity {
    ...
    private void checkLoginInServer(){
        startLoadingStatus();
        String requestJson = AeasyRequestUtil.getCheckLoginRequest(AeasySharedPreferencesUtil.getUserCode(this), AeasySharedPreferencesUtil.getUuid(this));
    ...
    public void onResponse(RequestWrapper response) {
        stopLoadingStatus();
        if(response != null && !TextUtils.isEmpty(response.getAeasyapp().getResponseBody().getUuid())){
            //保存
//            AeasySharedPreferencesUtil.saveUserCode(this, response.getUsercode());
            AeasySharedPreferencesUtil.saveUuid(this, response.getAeasyapp().getResponseBody().getUuid());
            Toast.makeText(this, getResources().getString(R.string.inspect_checklogin_success), Toast.LENGTH_SHORT).show();
            //跳转主界面界面
            ...
            //失败跳转登陆界面
            if(response != null && !TextUtils.isEmpty(response.getAeasyapp().getResponseCode()) && response.equals(AeaConstants.RESPONSE_CODE_600)){
                Toast.makeText(this, getResources().getString(R.string.inspect_checklogin_fail_timeout), Toast.LENGTH_SHORT).show();
            }else{
                Toast.makeText(this, getResources().getString(R.string.inspect_checklogin_fail), Toast.LENGTH_SHORT).show();
            }
